...
if($db->connect_errno==0){
	...
	// pagination
	$perpage = 5;
	$hasil = $db->query("SELECT * FROM jenis");
	...
	if(isset($_GET["tblCari"])){
		...
		$sql="SELECT id_jenis, nama_jenis, nama_pegawai
        FROM jenis LEFT JOIN pegawai ON jenis.id_pegawai = pegawai.id_pegawai
		WHERE nama_jenis LIKE '%$dicari%' 
		OR id_jenis LIKE '%$dicari%'
		OR nama_pegawai LIKE '%$dicari%'
		ORDER BY id_jenis";
	}   
	else{
	$sql="SELECT id_jenis, nama_jenis, nama_pegawai FROM jenis LEFT JOIN pegawai 
    ON jenis.id_pegawai = pegawai.id_pegawai ORDER BY id_jenis LIMIT $start, $perpage";
	}
	$res=$db->query($sql);
	if($res){
		?>
		<a href="jenis-tambah.php"><button type="button" class="btn btn-outline-primary rounded">Tambah Jenis</button></a>
		...
		foreach($data as $barisdata){ // telusuri satu per satu
			?>
			<tr>
			<td><?php echo $barisdata["id_jenis"];?></td>
			<td><?php echo $barisdata["nama_jenis"];?></td>
            <td><?php echo $barisdata["nama_pegawai"];?></td>
			<td class="text-center">
                    <a href="jenis-form-ubah.php?id_jenis=<?php echo base64_encode($barisdata["id_jenis"]); ?>">
                    <button type="button" class="btn btn-success">Ubah</button></a>
                    <a href="jenis-konfirmasi-hapus.php?id_jenis=<?php echo base64_encode($barisdata["id_jenis"]); ?>">
