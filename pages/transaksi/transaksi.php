...
if($db->connect_errno==0){
	...
	// pagination
	$perpage = 5;
	$hasil = $db->query("SELECT * FROM transaksi");
	...
	if(isset($_GET["tblCari"])){
		...
		$sql="SELECT id_transaksi, tanggal, deskripsi, id_barang, id_pembeli, nama_pegawai
        FROM transaksi LEFT JOIN pegawai ON transaksi.id_pegawai = pegawai.id_pegawai
		WHERE id_transaksi LIKE '%$dicari%' 
		OR tanggal LIKE '%$dicari%'
		OR deskripsi LIKE '%$dicari%'
		OR id_barang LIKE '%$dicari%'
		OR id_pembeli LIKE '%$dicari%'
		OR nama_pegawai LIKE '%$dicari%'
		ORDER BY id_transaksi ASC LIMIT $start, $perpage";
	}   
	else{
	$sql="SELECT id_transaksi, tanggal, deskripsi, id_barang, id_pembeli, nama_pegawai
	 FROM transaksi LEFT JOIN pegawai ON transaksi.id_pegawai = pegawai.id_pegawai
	...
	if($res){
		?>
		<a href="transaksi-tambah.php"><button type="button" class="btn btn-outline-primary rounded">Tambah Transaksi</button></a>
		...
		foreach($data as $barisdata){ // telusuri satu per satu
			?>
			<tr>
			<td><?php echo $barisdata["id_transaksi"];?></td>
			...
			<td class="text-center">
                    <a href="transaksi-form-ubah.php?id_transaksi=<?php echo base64_encode($barisdata["id_transaksi"]); ?>">
                    <button type="button" class="btn btn-success">Ubah</button></a>
                    <a href="transaksi-konfirmasi-hapus.php?id_transaksi=<?php echo base64_encode($barisdata["id_transaksi"]); ?>">
